/*
 * Address Vulnerability Check — ECDSA Nonce-Reuse Scanner
 * Cloned from the CryptographyTube TXID -> R,S,Z recovery engine.
 * Self-contained: depends only on CryptoJS (crypto-js) which is loaded
 * before this file. Exposes window.AddressVulnCheck.analyze() and
 * window.exportOne().
 *
 * How it works: given a Bitcoin address (or a raw TXID), it lists the
 * address's spending transactions, reconstructs the ECDSA signature
 * components (R, S, Z) for each input the address signed, and — if two
 * inputs reuse the same R value (nonce) with different S — algebraically
 * recovers the private key. A clear SAFE / VULNERABLE verdict is shown.
 */
(function () {
    "use strict";
    ...
    // (.avc-spin, not .fa-icon) so the global icon-tint filters never recolour it;
    // it is tinted + spun by tool.css.
    function spinner(size) {
        const n = size || 13;
        return '<img src="../assets/svgs/solid/spinner.svg" class="avc-spin" width="' + n +
            '" height="' + n + '" alt="loading">';
    }
    function setStats(stats) {
        const b = document.getElementById('avc-stat-balance');
        const r = document.getElementById('avc-stat-received');
        const t = document.getElementById('avc-stat-tx');
    ...
    function statLoading() {
        ['avc-stat-balance', 'avc-stat-received', 'avc-stat-tx'].forEach(function (id) {
    ...
    function getOutputHeader(elementId) {
        let header = `<div class="result-header">CRYPTOGRAPHYTUBE<span class="tool-tag">Address Vulnerability Scan</span></div>`;
        if (elementId) {
            header += `
    ...
    function exportOne(id, type) {
        ...
        headerText += `TOOL: ADDRESS VULNERABILITY CHECK\n`;
        ...
        if (type === 'txt') {
            ...
            a.href = url;
            a.download = `CryptographyTube_AddressVulnCheck_${id}.txt`;
    ...
    function renderResults(rsz, input, mode) {
        if (!rsz || rsz.length === 0) {
            return '<div class="avc-verdict safe"><span class="avc-verdict-mark">✓</span>' +
                '<span><strong>NOTHING TO CHECK</strong> — no spent inputs with ECDSA signatures were found for this ' +
                (mode === 'address' ? 'address' : 'transaction') +
                ', so there is no nonce-reuse exposure to analyze.</span></div>';
        }
        ...
        const recovered = vulns.filter(function (v) { return v.pk && v.pk.indexOf('ERROR') === -1; });

        let html = '';

        // Headline verdict banner — this tool's primary answer.
        if (recovered.length) {
            html += '<div class="avc-verdict vuln"><span class="avc-verdict-mark">⚠</span>' +
                '<span><strong>VULNERABLE</strong> — ' + recovered.length + ' private key' + (recovered.length > 1 ? 's' : '') +
                ' recoverable from reused ECDSA nonces (R-value reuse). The recovered key' + (recovered.length > 1 ? 's are' : ' is') +
                ' shown below.</span></div>';
        } else {
            html += '<div class="avc-verdict safe"><span class="avc-verdict-mark">✓</span>' +
                '<span><strong>SAFE</strong> — no ECDSA nonce reuse detected across ' + rsz.length + ' signature' + (rsz.length > 1 ? 's' : '') +
                ' scanned. No private key is recoverable this way.</span></div>';
        }
        ...
        if (recovered.length) {
            html += '<span class="rsz-chip rsz-chip-danger">R-reuse · ' + recovered.length + ' key' + (recovered.length > 1 ? 's' : '') + ' recovered</span>';
        } else {
    ...
    async function analyze() {
        const input = document.getElementById("avc_input").value.trim();
        const out = document.getElementById("avc_out");
        if (!input) return;

        // How many transactions to scan (address mode). Free entry — any number.
        const limitEl = document.getElementById("avc-tx-limit");
        ...
        setStatus('Analyzing input (address or TXID)…');
        ...
        try {
            ...
            if (input.length === 64 && /^[0-9a-fA-F]+$/.test(input)) {
                ...
                throw new Error("Invalid input. Paste a Bitcoin address (or a 64-character TXID).");
            }
    ...
    // ---- Expose globals used by the inline wiring + onclick handlers -----
    window.AddressVulnCheck = { analyze: analyze, setStats: setStats };
